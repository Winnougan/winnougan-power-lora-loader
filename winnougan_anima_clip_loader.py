...
class Qwen35HybridModel(nn.Module):
    ...
    def __init__(self, config_dict=None, dtype=None, device=None, operations=None):
        ...
        # comfy.sd1_clip.SDClipModel does `self.num_layers = self.transformer.num_layers`
        # so the transformer must expose num_layers as an instance attribute.
...
class WinnouganAnimaCLIPLoader:
    ...
    def load_clip(self, clip_name, use_calibration=False, use_alignment=False,
        ...
        # The CLIP loading chain dispatches through:
        #   CLIP.load_sd → SD1ClipModel.load_sd → SDClipModel.load_sd
        #   → self.transformer.load_state_dict(sd, strict=False)
        # So keys must match Qwen35HybridModel's parameter names directly:
        #   embed_tokens.weight, layers.0.input_layernorm.weight, norm.0.weight, etc.
        # The safetensors file may have a "model." HuggingFace prefix to strip.

        sample_keys = list(sd.keys())[:5]
        log.info(f"[{NODE_NAME}] Loaded {len(sd)} keys from {clip_name}")
        log.info(f"[{NODE_NAME}] Sample keys: {sample_keys}")
        ...
        if has_model_prefix:
            ...
            for k, v in sd.items():
                ...
                remapped[stripped] = v
            sd = remapped
            log.info(f"[{NODE_NAME}] Stripped 'model.' prefix from {len(sd)} keys. "
                     f"Sample: {list(sd.keys())[:3]}")

        # Detect dtype from norm weights
        detected_dtype = None
        for key in ("norm.1.weight",
                    "layers.0.input_layernorm.weight",
                    "embed_tokens.weight"):
            if key in sd:
                detected_dtype = sd[key].dtype
                log.info(f"[{NODE_NAME}] Detected dtype={detected_dtype} from key '{key}'")
                break

        if detected_dtype is None:
            log.warning(f"[{NODE_NAME}] Could not detect dtype from known keys — falling back to None")
        ...
        # Validate the CLIP loaded successfully
        if clip is None or not hasattr(clip, 'cond_stage_model'):
            raise RuntimeError(f"[{NODE_NAME}] CLIP object failed to initialize — "
                               f"check that '{clip_name}' is a valid Qwen3.5-4B text encoder.")

        # Diagnostic: verify weight loading succeeded (meta-tensor safe)
        try:
            test_param = clip.cond_stage_model.qwen35_4b.transformer.embed_tokens.weight
            if test_param is not None:
                log.info(f"[{NODE_NAME}] Weight verification passed — embed_tokens found "
                         f"(shape={test_param.shape}, dtype={test_param.dtype}, "
                         f"device={test_param.device})")
            else:
                log.error(f"[{NODE_NAME}] embed_tokens.weight is None — weights did not load")
        except AttributeError as e:
            log.warning(f"[{NODE_NAME}] Could not verify weights: {e}")
